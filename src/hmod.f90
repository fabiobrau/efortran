MODULE HMOD
    ...
    ! We assume not structured matrix (the black box multiplier is full
...
contains
    ...
    END FUNCTION LTRONC
    SUBROUTINE HODLRSVD(A,AZIP,K,LL,N0,T)
        ! with not efficient multiplication in sparse cases
        ...
        ! local variables
        double precision, dimension(N0*2**LL,K) :: U, V
        double precision, dimension(2**LL ,K) :: B
        type(HODMAT), dimension(LL) :: BZIP
        double precision :: TOL=1d-14
        integer :: I, J, L, N , R, NA, NB, II, JJ
        R=K
        N=size(A,1)
        do L=1,LL ! loop sui livelli 
            ...
            do J=1,2**L-1,2 !II,II+1 sibling iif I odd
                I=2**L -2 +J
                call svd(sub(A,I,I+1,T),U(T(I,1):T(I,2),:),V(T(I+1,1):T(I+1,2),:),B(J,:),K,TOL)
                call svd(sub(A,I+1,I,T),U(T(I+1,1):T(I+1,2),:),V(T(I,1):T(I,2),:),B(J+1,:),K,TOL)
            end do
            AZIP(L)=hodmat(U,V,B(1:2**L,:))
        end do
    END SUBROUTINE HODLRSVD
    SUBROUTINE HODLRRS(A,AZIP,K,P,LL,N0,T)
        ...
        type(HODMAT), dimension(LL) , intent(OUT):: AZIP
        ! local variables
        double precision, dimension(N0*2**LL,K+P) :: O, Y, Z, U, V
        double precision, dimension(2**LL ,K+P) :: B
        type(HODMAT), dimension(LL) :: BZIP
        double precision :: TOL=1d-14
        double precision, dimension(K+P,K+P) :: UAUX
        double precision, dimension(K+P) :: S
        integer :: I, J, L, N , R, NA, NB, II, JJ
        R=K+P
        N=size(A,1)
        do L=1,LL ! loop sui livelli 
            ...
            do J=1,2**L-1,2 !II,II+1 sibling iif I odd
                I=2**L -2 +J
                !! RANDOM SAMPLING
                Y(T(I,1):T(I,2),:)=matmul(sub(A,I,I+1,T),sub(O,I+1,0,T))
                Y(T(I+1,1):T(I+1,2),:)=matmul(sub(A,I+1,I,T),sub(O,I,0,T))
                !! ORTOGONALIZE RANDOM SAMPLING
                U(T(I,1):T(I,2),:)=qr(Y(T(I,1):T(I,2),:))
                U(T(I+1,1):T(I+1,2),:)=qr(Y(T(I+1,1):T(I+1,2),:))
                !! EXTRACT V AND B
                Z(T(I,1):T(I,2),:) = matmul(transpose(sub(A,I,I+1,T)),sub(U,I,0,T))
                Z(T(I+1,1):T(I+1,2),:)=matmul(transpose(sub(A,I+1,I,T)),sub(U,I+1,0,T))
                !! SVD
                call svd(sub(Z,I+1,0,T),V(T(I,1):T(I,2),:),UAUX,S,K,TOL)
                U(T(I+1,1):T(I+1,2),:)=matmul(U(T(I+1,1):T(I+1,2),:),UAUX)
                B(J+1,:)=S                   
                call svd(sub(Z,I,0,T),V(T(I+1,1):T(I+1,2),:),UAUX,S,K,TOL)
    ...
    SUBROUTINE BBMHODLR(A,AZIP,K,P,LL,N0,T)
        ! LL, scalar, integer           - number of level
        ! N0, scalar, integer           - N/2**LL
        ! P, scalar, integer,           - oversamplig parameter
        ! K, scalar, integer            - rank off daigonal blocks bound
    ...
    END SUBROUTINE BBMHODLR
    SUBROUTINE  HLRRS(A,HA,K,P,LL,N0,T)
        ...
        do J=1,2**LL
            I=2**LL-2+J
            ENDD=xdim(T,I)
            D(T(I,1):T(I,2),1:ENDD)=sub(A,I,I,T)
        end do
